...
test('counter should not go bellow 0', () => {
  const counter = 0;
  const wrapper = setup(null, { counter });
  ...
  expect(counterDisplay.text()).not.toContain(counter - 1);
});

test('should display error message when trying to decrement counter bellow 0', () => {
  const counter = 0;
  const error = false;
...
test('should clear error message when increments from 0', () => {
  const counter = 0;
  const error = true;
  const wrapper = setup(null, { counter, error });
